...
class Database extends mysqli {
	...
	private function logQuery( $query, $resultmode, $success, $backtrace ) {
		...
		/*$q = "INSERT INTO {$this->logdb}.sv_querylog (query, resultmode, success, backtrace, session) " .
		     "VALUES (" .
		       "'" . $this->real_escape_string($query) . "', " .
		       "'" . $this->getResultmodeString($resultmode) . "', " .
		       $success . ", " .
		       "'" . $this->real_escape_string(serialize($backtrace)) . "', " .
		       "'" . $this->real_escape_string(serialize($this->session)) . "'" .
		     ");";
		if( !parent::query( $q ) ) {
