...
if($kat=$_GET["kat"]) $search1=" gpid_demo=".$kat;
if($expid=$_GET["expid"])  $search2=" id=".$expid;

echo "Uuendan ... <br>";
//$line_temp["id"]=$_GET["id"];
$query_temp="SELECT * FROM exp WHERE ".$search1."".$search2;
		echo $query_temp."<br>";
...
// Väljundi kirjutamine väljale kokku_est - see on selle õpiobjekti kontekstist sõltumatu väljanägemine
  ...
  <tr>";
						  if($line_vahh1["veeb_pilt_url"])
										{
											  $sammud=$sammud."<td width=\"5%\"><img alt=\"".$line_vahh1["nimi_est"]."\" style=\"border-width:0\" src=\"".urldecode($line_vahh1["veeb_pilt_url"])."\" /></td>";
										}
    
    					$sammud=$sammud."<td valign=\"top\" width=\"99%\">".$text_out."</td>
  </tr>
</table>";
					}
					else
			...
			case 9: //pilt
//			echo $line_temp["veeb_pilt_url"];
				$mudel="<table align=\"center\" cellpadding=\"5\" width=\"100%\" border=\"0\">
  <tr>";
				if($line_temp["veeb_pilt_url"])
				{
					  $mudel=$mudel."<td width=\"5%\"><img alt=\"".$line_temp["nimi_est"]."\" style=\"border-width:0\" src=\"".urldecode($line_temp["veeb_pilt_url"])."\" /></td>";
				}
   $mudel=$mudel."<td valign=\"top\" width=\"95%\">".$line_temp["kirjeldus_est"]."</td>
  </tr>
</table>
";
				$valjund = 	$mudel;
...
";
			...
			case 32: 
				...
				$result_liik=mysql_query($query_liik);
				$liik=mysql_fetch_array($result_liik); 
				$mudel=$line_temp["probleem_est"];
// paneme lahenduse juurde? Või ei pane? Ikkagi paneme, variant, kus vajutamise peale hakkab vihjeid saama jääb Variandile, mis on praegu alajaotuste lõpus ...
			...
			case 40:
				$sammud = NULL;
				$sammud=$sammud."<p>";
				$sammud=$sammud."&Uuml;lesanne: ".$line_temp["probleem_est"];
				$sammud=$sammud."</p>";
			
				$query_vahh="SELECT * FROM exp_exp WHERE oid1=".$line_temp["id"]." AND meta1=2 order by order1";
				//echo $query_vahh;
				$result_vahh=mysql_query($query_vahh);
				$count_row=0;
				while($line_vahh=mysql_fetch_array($result_vahh))
				{
					...
					$query_vahh1="SELECT kokku_est FROM exp WHERE id=".$line_vahh["oid2"];
					//echo $line_vahh["oid2"]."".$query_vahh1;
					$result_vahh1=mysql_query($query_vahh1);
					$line_vahh1=mysql_fetch_array($result_vahh1);
					$sammud=$sammud."Lahendus: ".$line_vahh1["kokku_est"];
					$count_row++;
...
$query1414="UPDATE exp SET kokku_est='".$db->real_escape_string($valjund)."' WHERE id=".$line_temp["id"];
//echo $query1414;
$result1414=mysql_query($query1414);
$query1415="UPDATE exp SET kokkuprint_est='".$db->real_escape_string($valjund_print)."' WHERE id=".$line_temp["id"];
