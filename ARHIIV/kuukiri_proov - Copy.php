...
<body  bgcolor="#eeeeee" text="#000000">
  ...
  <tr>
    <td colspan="2" bgcolor="#CCCCCC" class="kiri_pealkiri">Kalendrijutt <span class="kiri">(<a href="http://www.fyysika.ee/teated" class="kiri">Vaata ka FYYSIKA.EE kalendrit</a>)</span></td>
    </tr>
  <tr>
...
<?
  ...
  <tr>
    <td colspan="2" bgcolor="#CCCCCC" class="kiri_pealkiri">FYYSIKA.EE uudisteportaal: teadusuudised oktoobris</td>
  </tr>
  <tr>
    <td colspan="2" class="kiri_pealkiri"><table width="100%" border="0" cellpadding="10">
        <tr>
          <td bgcolor="#FFFFFF" class="kiri"><?
$temp=array();
$temp1=array();
$temp2=array();

					$query_blogi2 ="SELECT * FROM wp_posts WHERE post_status='publish' AND post_type='post' AND `post_title` NOT LIKE  '%Eesti Füüsika Selts: ".$k_kuu."%' ORDER BY post_date DESC LIMIT 50";
//					echo $query_blogi2,"</br></br>";
					$result_blogi2=mysql_query($query_blogi2);
...
//teadusuudised välja ...
						$on_teadusuudis=0;
						$query_blogi1 = "SELECT * FROM wp_term_relationships WHERE object_id='".$line_blogi2["ID"]."' ORDER BY object_id DESC LIMIT 5";
						//echo $query_blogi1;
						$result_blogi1=mysql_query($query_blogi1);
						while($line_blogi1=mysql_fetch_array($result_blogi1))
						{
							if($line_blogi1["term_taxonomy_id"]==16) $on_teadusuudis=1;
							//echo "teadusuudis! <br>";
						}
						...
						$algus=$line_blogi2["post_date"];
//						echo $algus," ja ",date("Y-m-d",mktime(0, 0, 0, date("m")-1, date("d"),   date("Y")))," --- ";
						if($algus>$vahemik_algus&&$algus<$vahemik_lopp&&$on_teadusuudis==1)
						{
								$kuu=substr($algus,5,2);
								$paeva=substr($algus,8,2);
								$kuua = kuud($kuu,1);			
								...
								$aeg=$paeva.".".$kuu.".".$aasta."";
      	//echo $aeg;?>
            <a href="http://www.fyysika.ee/uudised/?p=<? echo $line_blogi2["ID"];?>" class="kiri"><? echo  $line_blogi2["post_title"]; ?></a>  <? echo " * ";
						}
...
//		echo $query_exp;
  ...
  <tr>
    <td colspan="2" bgcolor="#eeeeee" class="kiri"><div align="center"><table width="100%" border="0" cellpadding="10">
        <tr>
          <td class="kiri"><div align="center">Kirjuta meile efs@fyysika.ee. 
