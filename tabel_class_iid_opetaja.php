<?  function tabel_opetaja($domain,$oid,$login,$sel,$lisa,$aine){
	switch ($sel)
	{
	case 1:	$query2="SELECT id,oid2 FROM ".$domain." WHERE oid1=".$oid." AND sisu2='".$aine."'";
		break;
	case 2:	$query2="SELECT id,oid1 FROM ".$domain." WHERE oid2=".$oid." AND sisu2='".$aine."'";
		break;
	}
// mittes�mmeetrilisus, mis tuleneb juhust exp_exp, kus peaks lugema nii 
...
<table width="100%" border="0" cellspacing="1" cellpadding="0">
		<tr><td width="93%" background="images/sinine.gif" class="menu">Nimi<input type="button" name="suva2" class="button" value="Otsi ja lisa" onclick="window.open('addvalue_iid_asp.php?domain=isik_kool&amp;oid=<? echo $oid;?>&amp;sel=<? echo $sel;?>&amp;otsing=isik&vali=perenimi&aine=<?echo $aine; ?>','File_upload','toolbar=0,width=1200,height=680,status=yes');">
		</td>
...
$temp=array();
	...
	$count++;
			  ...
			  ?> <tr align="left" >
				  ...
				  if($aine=='nublu')
				  	{
				  ?><input type="button" name="suva1" class="button" value="Otsi olemasolevatest" onclick="window.open('addvalue_iid_tugikool.php?domain=<? echo $domain;?>&oid=<? echo $oid;?>&sel=<? echo $sel;?>&aine=<? echo $aine;?>','File_upload','toolbar=0,width=660,height=580,status=yes');" >
				  
			        <? //echo "�ige koht";
					}
					else
					{
				  ?><input type="button" name="suva1" class="button" value="Otsi olemasolevatest" onclick="window.open('addvalue_iid_opetaja.php?domain=<? echo $domain;?>&oid=<? echo $oid;?>&sel=<? echo $sel;?>&aine=<? echo $aine;?>','File_upload','toolbar=0,width=660,height=580,status=yes');" >
