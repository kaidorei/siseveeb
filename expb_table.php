...
// kui on tehtud valik demolisuse j'rgi .. 
	...
	$query="SELECT nimi_est,nimi_eng,id,staatus_id,lupdate,veeb_pilt_url,veeb_pilt_url_s, naita_veebis, veeb_ok FROM exp WHERE nimi_est LIKE '%".$_POST["search_str"]."%'".$str." ".$str1." ".$str2." AND in_buss=1 ORDER BY nimi_est";
	
	
	if($_POST["search_YTID"])	{
	$query="SELECT nimi_est,video_player,nimi_eng,id,staatus_id,lupdate,veeb_pilt_url,veeb_pilt_url_s, naita_veebis, in_buss FROM exp WHERE video_url LIKE '%".$_POST["search_YTID"]."%'".$str." ".$str1." ".$str2." ORDER BY nimi_est";
	}
