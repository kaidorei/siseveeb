...
format: function(i) {
	if (this.data[i] == null)
		...
		return '<IMG STYLE="cursor: pointer;" SRC="crystal/' + this.data[i] + '-small.png" TITLE="' + T[this.data[i]] + '" onclick="timg(\'' + this.data[i] + '\',\'crystal/' + this.data[i] + '.png\')">';
		//return '<A HREF="crystal/' + this.data[i] + '.png" TARGET="_blank" TITLE="' + T[this.data[i]] + '"><IMG SRC="crystal/' + this.data[i] + '-small.png"></A>';
},
...
{
source: ("http://en.wikipedia.org/ wiki/ Timeline_of_chemical_ elements_discoveries").blank(),
format: function(i) {
	if (this.data[i] == null)
		return T.antique;
	else
...
[
 ...
 "Antoine Jérôme Balard|Leopold Gmelin",//Br
...
format: function(i) {
	if (this.data[i] == null)
		...
		var f = function(s) { return s.split(" ").last().link({ blank: true, title: T.wikiart + ": " + s, url: s.toWikiUrl() }); };
		return this.data[i].split("|").apply(f).join("<br>");
	}
}
}
);