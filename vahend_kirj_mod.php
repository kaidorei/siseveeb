...
//	  echo "nupp=",$asi;
	  ...
	  // kontrollime kasutamise üle ...
		$query_c="SELECT ".$vali."_check from vahendid WHERE id='".$id."'";
		$result_c=mysql_query($query_c);
		$line_c=mysql_fetch_array($result_c);
	  	$tehtud=0;		
		if ($line_c[$vali."_check"]<1 or $line_c[$vali."_check"]==$isik_id) // teine valik on juhuks, kui eksinud kasutaja tagasi pöördub ...
		{
	  // check out ...
		$query_c="UPDATE vahendid SET ".$vali."_check=".$isik_id." WHERE id='".$id."'";
		$result_c=mysql_query($query_c);
		
		}else{
		echo $isik_nimi," justkui töötab selle teksti kallal. Oled Sa kindel?";
		}
		...
		if($_GET["act"]=="upi"){	
			if($nupp=="Salvesta") // salvestan vaid juhul, kui vastavat nuppu vajutatakse, muidu mitte (katkesta)
			{
				echo "uuendan ... ";
				$var=$_POST;
				$query="UPDATE vahendid SET ".$vali."='".$_POST[$vali]."' WHERE id=".$id;
				echo $query;
				$result=mysql_query($query);
			}
			
			// check out ...
			$query_c="UPDATE vahendid SET ".$vali."_check='' WHERE id='".$id."'";
			$result_c=mysql_query($query_c);
...
//vastavalt seose id-le leian tabelist seose kaks otsa
	$query="SELECT nimi_est, ".$vali." FROM vahendid WHERE id=".$id;
	$result=mysql_query($query);
